...
- Seed: SUPER_ADMIN + STORE_OWNER + ADMIN + MANAGER + EMPLOYEE + CUSTOMER. SUPER_ADMIN all perms.
...
1. `JwtGuard` — verify token, load user fresh + role.permissions relations, set `req.user = {id,name,email,role:{id,key}|null,permissions[]}`. Role change live next request (no token role trust).
